...
import com.footprint.app.FirebaseDatabaseManager.savePetinfodata
...
class membershipActivity : AppCompatActivity() {
    ...
    // 멤버 변수로 이미지 Uri 저장
    private var selectedPersonImageUri: Uri? = null
    private var selectedDogImageUri: Uri? = null
    private var gallerystate: Boolean = false
...
//        binding.cvDogimage.setOnClickListener {
//            gallerystate = true
//            openGallery()
//        }
...
//                return@setOnClickListener
//            }
            uploadImage(selectedPersonImageUri) { imagePersonUrl ->
                saveProfiledata(
                    ProfileModel(
                        nickName = nickName,
                        address = binding.townPosition.text.toString(),
                        introduction = binding.introduction.text.toString(),
                        profileImageUri = imagePersonUrl
                    )
                ) {
                    uploadImage(selectedDogImageUri) { imagePetUrl ->
                        savePetinfodata(
                            PetInfoModel(
                                timestamp = System.currentTimeMillis(),
                                petImageUrl = imagePetUrl,
                                petName = dogName,
                                petAge = dogAge.toString(),
                                petSex = when (binding.dogSex.checkedRadioButtonId) {
                                    R.id.man -> "남아"
                                    R.id.woman -> "여아"
                                    else -> ""
                                },
                            ...
                            saveProfiledata(
                                ProfileModel(
                                    ...
                                    introduction = binding.introduction.text.toString(),
                                    profileImageUri = imagePersonUrl,
                                    petInfoModelList = mutableListOf(petInfoData!!)
                                )
                            ) {
                                //MainActivity로 이동
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_GALLERY && resultCode == RESULT_OK) {
            ...
            if (!gallerystate) {
                selectedPersonImageUri = imageUri // 선택된 이미지 Uri 저장
                binding.profileImage.setImageURI(imageUri)
            } else {
                selectedDogImageUri = imageUri
//                binding.ivDogimage.setImageURI(imageUri)
            }
        }
    }
